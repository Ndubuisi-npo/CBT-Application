  ...
  <div class="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
          ...
          <table class="min-w-full divide-y divide-slate-200 bg-white">
            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="teacher in teachersStore.teachers" :key="teacher.id" class="transition hover:bg-slate-50/80">
                ...
                <td class="px-5 py-4">
                  <button type="button" class="sa-button-secondary px-3 py-2 text-xs" @click="editTeacher(teacher)">Edit</button>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <SectionCard :title="form.id ? 'Edit Teacher' : 'Create Teacher'" subtitle="Create staff records and assign classes and subjects.">
      <form class="space-y-5" @submit.prevent="submit">
        ...
        <button type="submit" class="sa-button-primary w-full">{{ form.id ? 'Update Teacher' : 'Create Teacher' }}</button>
      </form>
    </SectionCard>
  </div>
</template>
