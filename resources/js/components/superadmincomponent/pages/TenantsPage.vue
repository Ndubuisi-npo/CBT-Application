    ...
    <SectionCard title="Tenants" subtitle="Search, filter, and manage every school workspace from one place.">
      <template #header>
        <div class="flex flex-wrap items-center gap-3">
          <RouterLink to="/super-admin/tenants/new" class="sa-button-primary">
            <Plus class="h-4 w-4" />
      ...
      <div class="mb-6 grid gap-4 lg:grid-cols-[1fr_220px]">
        <div class="relative">
          ...
          <input v-model="tenantsStore.searchQuery" class="sa-input pl-12" placeholder="Search by school name or domain" />
        </div>
        <select v-model="tenantsStore.statusFilter" class="sa-input">
              ...
              <tr v-for="tenant in tenantsStore.paginatedTenants" :key="tenant.id" class="transition hover:bg-slate-50/80">
                ...
                <td class="px-5 py-4">
                  <div class="flex flex-wrap gap-2">
                    <button type="button" class="sa-button-secondary px-3 py-2 text-xs" @click="notify('View tenant details is ready for API hookup.')">View</button>
                    <button type="button" class="sa-button-secondary px-3 py-2 text-xs" @click="notify('Edit tenant workflow is ready for API hookup.')">Edit</button>
