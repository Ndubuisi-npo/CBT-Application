    ...
    <div class="mx-auto max-w-5xl">
      ...
      <div v-if="feature" class="mt-10 rounded-[32px] bg-white p-10 shadow-[0_30px_80px_rgba(15,23,42,0.08)]">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p class="text-sm uppercase tracking-[0.28em] text-amber-500">Feature</p>
          ...
          <div class="rounded-3xl bg-amber-50 px-6 py-5 text-center">
            <p class="text-xs uppercase tracking-[0.28em] text-amber-700">Key benefit</p>
            <p class="mt-2 text-xl font-semibold text-amber-900">{{ feature.benefit }}</p>
          </div>
        </div>

        <div class="mt-12 grid gap-6 lg:grid-cols-2">
          <div v-for="section in feature.sections" :key="section.title" class="rounded-3xl border border-slate-200 bg-slate-50 p-7">
            <h2 class="text-xl font-semibold text-slate-900">{{ section.title }}</h2>
            <p class="mt-3 text-slate-600">{{ section.description }}</p>
          </div>
        </div>

        <div class="mt-12 rounded-3xl border border-slate-200 bg-white p-8">
          <h2 class="text-xl font-semibold text-slate-900">How it works</h2>
          <ol class="mt-6 space-y-5 text-slate-600">
            <li v-for="step in feature.steps" :key="step.title" class="space-y-2 rounded-2xl bg-slate-50 p-5">
